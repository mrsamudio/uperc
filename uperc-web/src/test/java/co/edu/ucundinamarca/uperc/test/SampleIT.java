...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "uperc-ear.ear");

        // Current directory is the root of the "uperc-web" project. Go up one level
        // and enter the "uperc-ejb" project.
        // The ejb jar is found in the "target" directory:
        File f = new File("../uperc-ejb/target/uperc-ejb.jar");
        JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "uperc-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/uperc-web.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "uperc-web.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
        ...
        war.addPackage("co.edu.ucundinamarca.uperc.test");
